...
当定义一个类时，**通过定义5种特殊的成员函数控制这些操作，拷贝初始化（<font color="red">用等号定义变量</font>）通过拷贝构造函数或者移动构造函数实现的：**
...
本质上是函数，名字格式`returnType operator符号(参数)`。**某些运算符，包括赋值运算符（返回值是一个类型的yin'yong）必须定义为成员函数。**
